...
// ReSharper disable once EmptyEmbeddedStatement
        ...
        /**
         * This function is called when a connection from a freelancer player is broken.
         */

        public void dps_PlayerDestroyed(Session session, string reason)
        {
            _log.AddLog(LogType.GENERAL, "Player destroyed client={0} reason={1}", session.Client, reason);
            
            AddEvent(new DPSessionTerminatedEvent(session));
        }
    ...
    internal class DPSessionConnectedEvent : ReactorEvent
    {
        public Session dplayid;

        public DPSessionConnectedEvent(Session dplayid)
        {
            this.dplayid = dplayid;
        }
    }

    internal class DPSessionTerminatedEvent : ReactorEvent
    {
        public Session dplayid;

        public DPSessionTerminatedEvent(Session dplayid)
        {
            this.dplayid = dplayid;
        }
    }

    internal class DPSessionRxMessageFromClient : ReactorEvent
    {
        public Session dplayid;
        public byte[] msg;

        public DPSessionRxMessageFromClient(Session dplayid, byte[] msg)
        {
            this.dplayid = dplayid;
            this.msg = msg;
        }
