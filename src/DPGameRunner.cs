...
namespace FLServer
{
    ...
    public class DPGameRunner : Reactor
    {
        ...
        /// </summary>
        public Dictionary<uint, PlayerListItem> Playerlist = new Dictionary<uint, PlayerListItem>();
        ...
        public void GameThreadRun()
        {
            ...
            while (running)
            {
                ...
                else if (next_event is DPGameRunnerPlayerDeletedEvent)
                {
                    ...
                    if (!Players.ContainsKey(revent.FlPlayerID)) continue;
        ...
        public Ship.Ship FindShip(uint objid)
        {
            if (!Objects.ContainsKey(objid))
                return null;
            return (Ship.Ship)AffObjects[objid];
        }
        ...
        public void NotifyOnActivateCruise(Ship.Ship ship, bool state, uint dunno)
        {
            byte[] omsg = {0x0E, 0x01};
        ...
        public void NotifyOnActivateThrusters(Ship.Ship ship, bool state)
        {
            byte[] omsg = {0x15, 0x01};
            FLMsgType.AddUInt32(ref omsg, ship.Objid);
            FLMsgType.AddUInt8(ref omsg, state ? 1u : 0u);
        ...
        public void NotifyOnGoTradelane(Ship.Ship ship, uint ring1, uint ring2, uint dunno)
        {
            byte[] omsg = {0x0F, 0x01};
            FLMsgType.AddUInt32(ref omsg, ship.Objid);
        ...
        public void NotifyOnStopTradelane(Ship.Ship ship, uint ring1, uint ring2)
        {
            byte[] omsg = {0x10, 0x01};
            FLMsgType.AddUInt32(ref omsg, ship.Objid);
            FLMsgType.AddUInt32(ref omsg, ring1);
    ...
    internal class DPGameRunnerRxMsgEvent : ReactorEvent
    {
        public byte[] msg;
        public Player.Player player;

        public DPGameRunnerRxMsgEvent(Player.Player player, byte[] msg)
        {
            this.player = player;
            this.msg = msg;
        }
    ...
    internal class DPGameRunnerPlayerUpdateEvent : ReactorEvent
    {
        // This reference is thread safe but the contents may be changed by the owning
        // game thread. Don't look inside if you're not the owner.

        // These fields are owned by the event and can be safely accessed by anybody
        public uint flplayerid;
        public Group group;
        public Group groupInvited;
        public string name;
        ...
        public DPGameRunnerPlayerUpdateEvent(Player.Player player)
        {
            this.player = player;
            runner = player.Runner;

            flplayerid = player.FLPlayerID;
            name = player.Name;
            rank = player.Ship.Rank;
            system = player.Ship.System;
            @group = player.Group;
            groupInvited = player.GroupInvited;
        }
    }

    internal class DPGameRunnerPlayerDeletedEvent : ReactorEvent
    {
        public uint FlPlayerID;

        public DPGameRunnerPlayerDeletedEvent(uint flplayerid)
        {
