...
namespace FLServer
{
    ...
    {
        private static DPCInBaseState instance;
        ...
        public void EnterState(Player.Player player)
        {
            ...
            player.SendCharInfoRequestResponse();
        }
        ...
        private void RxCommonUpdateObject(Player.Player player, byte[] msg)
        {
        ...
        private void RxCommonFireWeapon(Player.Player player, byte[] msg)
        {
        ...
        private void RxSetTarget(Player.Player player, byte[] msg)
        {
        ...
        private void RxActivateEquip(Player.Player player, byte[] msg)
        {
        ...
        private void RxActivateCruise(Player.Player player, byte[] msg)
        {
        ...
        private void RxGoTradelane(Player.Player player, byte[] msg)
        {
        ...
        private void RxStopTradelane(Player.Player player, byte[] msg)
        {
        ...
        private void RxSetWeaponsGroup(Player.Player player, byte[] msg)
        {
        ...
        private void RxMunitionCollision(Player.Player player, byte[] msg)
        {
            ...
            if (targetObj != null)
            {
                ...
                if (munition != null)
                {
                    ...
                    if (targetObj is Ship.Ship)
                    {
                        var ship = targetObj as Ship.Ship;
        ...
        private void RxRequestLaunch(Player.Player player, byte[] msg)
        {
            ...
            player.SendMiscObjUpdate(Player.Player.MiscObjUpdateType.SYSTEM, player.FLPlayerID, player.Ship.System.SystemID);
            ...
            player.SendServerLaunch();

            player.SendMiscObjUpdate(Player.Player.MiscObjUpdateType.UNK3, player.Ship.Objid);
            player.SendMiscObjUpdate(Player.Player.MiscObjUpdateType.UNK2, player.Ship.Objid);
        }
        ...
        private void RxEnterBase(Player.Player player, byte[] msg)
        {
            ...
            player.SendSetHullStatus(player.Ship);
        }

        private void RxExitBase(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestBaseInfo(Player.Player player, byte[] msg)
        {
            ...
            if (type == 1)
            {
                player.SendSetStartRoom(baseid, UniverseDB.FindBase(baseid).StartRoomID); // fixme
                player.SendGFCompleteMissionComputerList(baseid);
                player.SendGFCompleteNewsBroadcastList(baseid);
            }
        }

        private void RxRequestLocationInfo(Player.Player player, byte[] msg)
        {
            ...
            if (type == 1)
            {
                player.SendGFCompleteCharList(roomid);
                player.SendGFCompleteScriptBehaviourList(roomid);
                player.SendGFCompleteAmbientScriptList(roomid);
            }
        }

        private void RxEnterLocation(Player.Player player, byte[] msg)
        {
        ...
        private void RxExitLocation(Player.Player player, byte[] msg)
        {
            // FLPACKET_CLIENT_ENTERLOCATION
            int pos = 2;
            uint roomid = FLMsgType.GetUInt32(msg, ref pos);
        ...
        private void RxRequestCreateShip(Player.Player player, byte[] msg)
        {
            ...
            if (player.Ship.Basedata != null)
            {
                var action = new LaunchFromBaseAction();
                action.DockingObj = player.Ship.Basedata.GetLaunchPoint(player.Ship);
            ...
            else
            {
                ...
                action.Position = player.Ship.Position;
                action.Orientation = Quaternion.MatrixToQuaternion(player.Ship.Orientation);
                player.Ship.CurrentAction = action;
            }

            player.SendCreateShipResponse(player.Ship);
        }


        private void RxSetVisitedState(Player.Player player, byte[] msg)
        {
        ...
        private void RxJettisionCargo(Player.Player player, byte[] msg)
        {
            ...
            if (jettisonedItem != null)
            {
                ...
                var newLoot = new Loot(lootCrate, 1.0f, jettisonedItem.arch, jettisonedItem.health, (ushort) count,
                ...
                player.SendSetEquipment(player.Ship.Items);
            }
            //TODO: Kick for cheating in else case
        }

        private void RxActivateThrusters(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestPlayerStats(Player.Player player, byte[] msg)
        {
        ...
        private void RxSetManeuver(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestBestPath(Player.Player player, byte[] msg)
        {
        ...
        private void ComputeBestPath(Player.Player player, uint spaceobjid, Waypoint[] waypoints)
        {
        ...
        private void RxRequestNavMap(Player.Player player, byte[] msg)
        {
            player.Log.AddLog(LogType.FL_MSG, "rx Send_FLPACKET_COMMON_REQUEST_NAVMAP");
        }

        private void RxSetInterfaceState(Player.Player player, byte[] msg)
        {
            player.Log.AddLog(LogType.FL_MSG, "rx Send_FLPACKET_COMMON_SET_INTERFACE_STATE");
        }

        private void RxRequestEvent(Player.Player player, byte[] msg)
        {
            ...
            if (solar.BaseData != null)
            {
                ...
                if (docking_obj == null)
                {
                    ...
                    player.SendServerRequestReturned(player.Ship, null);
                    return;
                }
                ...
                player.SendServerRequestReturned(player.Ship, docking_obj);
            }
        ...
        private void RxRequestCancel(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestSetHullStatus(Player.Player player, byte[] msg)
        {
            int pos = 2;
        ...
        private void RxLaunchComplete(Player.Player player, byte[] msg)
        {
        ...
        private void RxClientHail(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestUseItem(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestInvincibility(Player.Player player, byte[] msg)
        {
        ...
        private void RxJumpInComplete(Player.Player player, byte[] msg)
        {
        ...
        private void RxGoodSell(Player.Player player, byte[] msg)
        {
        ...
        private void RxGFSelectObject(Player.Player player, byte[] msg)
        {
            ...
            foreach (BaseCharacter ch in player.Ship.Basedata.Chars.Values)
            {
                if (ch.Type == "bar")
                {
                    player.SendNPCGFUpdateScripts(ch, 2);
                }
            }
        }

        private void RxMissionResponse(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestShipArch(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestEquipment(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestAddItem(Player.Player player, byte[] msg)
        {
            ...
            uint count = FLMsgType.GetUInt32(msg, ref pos);
            float health = FLMsgType.GetFloat(msg, ref pos);
            bool mounted = FLMsgType.GetUInt8(msg, ref pos) == 1 ? true : false;
            ...
            if (!mounted)
            {
                ...
                player.SendAddItem(goodid, item.hpid, count, health, mounted, hpname);
            }
            ...
            else
            {
                ...
                item.mission = false;
                player.Ship.Items[item.hpid] = item;
                player.SendAddItem(goodid, item.hpid, item.count, item.health, item.mounted, item.hpname);
            }
        ...
        private void RxRequestRemoveItem(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestSetCash(Player.Player player, byte[] msg)
        {
        ...
        private void RxRequestChangeCash(Player.Player player, byte[] msg)
        {
        ...
        private void RxSystemSwitchOutComplete(Player.Player player, byte[] msg)
        {
            ...
            if (player.Ship.CurrentAction is JumpAction)
            {
                var action = player.Ship.CurrentAction as JumpAction;
        ...
        public void RxObjectCollision(Player.Player player, byte[] msg)
        {
        ...
        private void HandleGroupLeaveRequest(Player.Player player)
        {
        ...
        private void HandleGroupInvitationAcceptedRequest(Player.Player player, Player.Player playerInviter)
        {
        ...
        private void HandleGroupInviteRequest(Player.Player playerFrom, Player.Player playerTo)
        {
            ...
            playerFrom.SendChatCommand(Player.Player.ChatCommand.GROUPINVITATIONSENT, playerTo.FLPlayerID);
            playerTo.SendChatCommand(Player.Player.ChatCommand.GROUPINVITATIONRECEIVED, playerFrom.FLPlayerID);
