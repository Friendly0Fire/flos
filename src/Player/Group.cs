...
namespace FLServer.Player
{
    public class Group
    {
        ...
        {
            playerJoined.SendChatCommand(Player.ChatCommand.GROUPJOINED, playerInviter.FLPlayerID);

            foreach (Player member in Members)
            {
                member.SendChatCommand(Player.ChatCommand.NEWGROUPMEMBER, playerJoined.FLPlayerID);

                if (member != playerInviter)
                {
                    playerJoined.SendChatCommand(Player.ChatCommand.NEWGROUPMEMBER, member.FLPlayerID);
                }
            }
        ...
        public void Leave(Player player)
        {
            ...
            player.SendChatCommand(Player.ChatCommand.GROUPLEFT, player.FLPlayerID);

            foreach (var member in Members)
            {
                member.SendChatCommand(Player.ChatCommand.GROUPMEMBERLEFT, player.FLPlayerID);
            }
        }
